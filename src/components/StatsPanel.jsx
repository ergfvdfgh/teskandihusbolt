...
export default function StatsPanel() {
  const { state } = useGame();
  const { money, items, day, history } = state;

  const totalItemsSold = history
    .filter(h => h.type === 'SELL')
    .reduce((sum, h) => sum + h.quantity, 0);
  
  const totalItemsBought = history
    .filter(h => h.type === 'BUY')
    .reduce((sum, h) => sum + h.quantity, 0);
  
  const totalUpgrades = history
    .filter(h => h.type === 'UPGRADE')
    .length;
  
  const totalTransactions = history.length;
  
  const mostProfitableItem = items.reduce((best, item) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow p-4">
      <h2 className="text-xl font-bold text-gray-800 mb-4">Statisztikák</h2>
      
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="bg-gradient-to-br from-green-50 to-emerald-100 border border-green-200 rounded-lg p-4">
          <div className="text-gray-600 text-sm mb-1">Napok száma</div>
          <div className="text-3xl font-bold text-green-700">{day}</div>
        </div>
        
        <div className="bg-gradient-to-br from-blue-50 to-cyan-100 border border-blue-200 rounded-lg p-4">
          <div className="text-gray-600 text-sm mb-1">Tranzakciók</div>
        ...
        <div className="bg-gradient-to-br from-purple-50 to-pink-100 border border-purple-200 rounded-lg p-4">
          <div className="text-gray-600 text-sm mb-1">Eladott termékek</div>
          <div className="text-3xl font-bold text-purple-700">{totalItemsSold}</div>
        </div>
        
        <div className="bg-gradient-to-br from-yellow-50 to-orange-100 border border-yellow-200 rounded-lg p-4">
          <div className="text-gray-600 text-sm mb-1">Vásárolt termékek</div>
          <div className="text-3xl font-bold text-yellow-700">{totalItemsBought}</div>
        </div>
      </div>
      
      <div className="mb-6">
        <h3 className="font-bold text-gray-700 mb-3">Teljesítmény mutatók</h3>
        <div className="space-y-4">
          <div>
            <div className="flex justify-between mb-1">
              <span className="text-gray-700">Hatékonyság</span>
              <span className="font-bold">{calculateEfficiency()}%</span>
            </div>
          ...
          <div>
            ...
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div
                ...
                style={{ width: `${Math.min(totalUpgrades * 10, 100)}%` }}
              ></div>
            </div>
            <p className="text-xs text-gray-500 mt-1">
          ...
          <div>
            <div className="flex justify-between mb-1">
              ...
              <span className="font-bold">{formatCurrency(money / day)}</span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div
                className="bg-blue-500 h-2 rounded-full"
                style={{ width: `${Math.min((money / day) / 100, 100)}%` }}
              ></div>
            </div>
            <p className="text-xs text-gray-500 mt-1">
              Átlagos napi profit
      ...
      <div className="bg-gradient-to-r from-gray-50 to-gray-100 border border-gray-200 rounded-lg p-4">
        ...
        <div className="flex items-center">
          ...
          <div className="ml-4">
            <div className="font-bold text-lg">{mostProfitableItem.name || 'Nincs adat'}</div>
            <div className="text-gray-600">
              Profit / db: <span className="font-bold text-green-600">{mostProfitableItem.profit} Ft</span>
            </div>
          </div>
        </div>
