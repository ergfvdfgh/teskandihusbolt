...
export default function ItemCard({ item }) {
  ...
  return (
    <div className={`border rounded-lg p-4 ${tierColors[item.tier]} border-2`}>
      <div className="flex justify-between items-start mb-2">
        <h3 className="text-lg font-bold">{item.name}</h3>
        <span className={`px-2 py-1 rounded text-xs font-bold ${tierTextColors[item.tier]}`}>
          Tier {item.tier}
        </span>
      </div>
      ...
      <div className="space-y-2 mb-4">
        <div className="flex justify-between">
          ...
          <span className="font-bold text-red-600">{item.buyPrice} Ft</span>
        </div>
        <div className="flex justify-between">
          <span className="text-gray-600">Eladási ár:</span>
          <span className="font-bold text-green-600">{item.sellPrice} Ft</span>
        </div>
        <div className="flex justify-between">
        ...
        <div className="flex justify-between">
          ...
          <span className="font-bold">Szint {item.upgradeLevel}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-gray-600">Profit / db:</span>
          <span className="font-bold text-blue-600">{item.sellPrice - item.buyPrice} Ft</span>
        </div>
      </div>
